...
| 系统稳定性与工程可复现性 | 20 | CLI、FastAPI 同步/异步接口、失败 trace 返回、trace、batch_report、部署文档、API 合约、Docker、测试、覆盖率、提交压缩包、案例 artifact、稳定性报告、API 并发 smoke、真实 HTTP loopback 压测、长文档分片执行 | 已在 HeyWhale CPU 环境验证；API 失败响应会返回 trace 路径；`submission_artifacts/stability/` 汇总 17 个保存案例的 trace、工具耗时、质量状态和恢复统计；`submission_artifacts/api_load_smoke/` 保存 8 请求并发 4 的本地接口 smoke；`submission_artifacts/http_load_test/` 保存 12 请求并发 6 的真实 HTTP loopback 混合接口压测；`submission_artifacts/http_load_test_100/` 保存 100 请求并发 20 的真实 HTTP loopback 混合接口压测；`submission_artifacts/long_document_chunks/` 保存 NIST 48 页在线 API 分片执行结果；`submission_artifacts/coverage/` 保存本地 pytest 行覆盖率；`Dockerfile` 和 `docker-compose.yml` 支持一键启动 API | 补外部公网/真实 GPU 高并发现场压测记录 |
