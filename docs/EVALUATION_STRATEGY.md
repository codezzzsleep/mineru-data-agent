...
3. 扩展大模型增强：当前 LLM 已参与解析前 profile/method/backend/lang 调度，并能把 fallback policy 写入执行计划；新运行还会把解析后风险写入 `recovery_decision.llm_quality_decision`。下一步用真实 key 复跑 10 个 LLM on/off 对比案例。
