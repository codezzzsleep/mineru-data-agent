...
- 不把第三方项目的成熟能力包装成本项目原创成果。
- 如果未来直接引入第三方代码，必须保留许可证、版权声明，并在技术报告中明确说明来源和改动范围。

`frondesce/mineru-kb-packager` 只作为“MinerU 后处理可以服务检索入库”的方向参考。本项目没有把该仓库作为依赖，也不把它包含在提交包中。为避免命名和叙事混淆，本项目将相关能力定位为 `Retrieval Exporter`，输出 `retrieval_chunks.jsonl`、`retrieval_manifest.json` 和 `retrieval_quality.json`，服务于本项目的可验证结构化结果与评审复查。
...
- 它是否增强 Agent 的自动决策、工具调用、异常恢复或稳定复现？
