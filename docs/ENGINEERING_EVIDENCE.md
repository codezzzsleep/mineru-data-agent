...
| --- | --- | --- |
| Task planning explainability is weak | `result.json.execution_control` records requested/initial/resolved controls, applied/ignored LLM recommendations, planning rationale, and `adaptive_decision` with task intents, target schema, post-processors, quality thresholds, and recovery strategy. `submission_artifacts/adaptive_cases/` proves the same input document produces different plans and `task_result` for growth ranking vs anomaly-evidence review. | It is deterministic adaptive planning plus optional LLM merge, not a full multi-turn conversation planner. |
