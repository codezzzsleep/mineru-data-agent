...
| Cost/speed/quality tradeoff is not quantified | `submission_artifacts/stability/stability_report.md` summarizes 17 saved cases, tool calls, tool elapsed seconds, recovery count, quality status, and provenance distribution. `submission_artifacts/baseline_comparison/baseline_comparison.md` groups saved artifacts by runner/scenario and compares labeled checks, quality, tool seconds, page provenance, and recovery. `submission_artifacts/http_load_test/http_load_test_report.md` adds a 12-request, concurrency-6 live HTTP loopback result. `submission_artifacts/llm_cost/llm_cost_report.md` audits LLM token/cost availability. | The comparison is based on saved artifacts and lightweight labels; the HTTP test is local loopback, not a public network/GPU/cloud-cost benchmark. Older LLM artifacts may lack provider token usage. |
