...
| Cost/speed/quality tradeoff is not quantified | `submission_artifacts/stability/stability_report.md` summarizes 17 saved cases, tool calls, tool elapsed seconds, recovery count, quality status, and provenance distribution. `submission_artifacts/evaluation/evaluation_metrics.md` summarizes labeled accuracy gates. | This is artifact-level stability evidence, not a high-concurrency load test or cloud cost benchmark. |
...
| Missing confidence/source evidence | Quality issues include evidence payloads; retrieval chunks include page/document provenance; numeric/table evaluation checks exact evidence fragments. | Most key-values do not yet have per-field confidence or bbox; online API path lacks page-level provenance. |
...
| API is not evaluation-friendly | `docs/API_CONTRACT.md` defines request fields, response schema, and structured error codes. API persists trace/result/summary paths after response. | No async job-status endpoint; long parses are synchronous. Public auth/rate limiting is expected at deployment gateway. |
